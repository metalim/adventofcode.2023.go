// почти верно. Осталась самая малость: понять значение offset. Оно ломает результат.
	...
	if len(results) > 0 {
		...
		for _, r := range results {
			if r.offset != baseOffset {
				fmt.Println("Offsets differ, need CRT for complex case")
				return
			}
		}
